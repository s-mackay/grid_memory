function [handles, results] = perm_multcompare_gm_publ(...
    condA, condB,nperms, clusteralpha, alpha, xaxis, plotcolors, ...
...
if ~legendoff
    [lh objh] = legend(lineh, conditionnames, 'Position', ...
...
for c = 1:length(pos_true.froms)
    ...
    k = plot(xaxis([pos_true.froms(c), pos_true.tos(c)]), [yl(1), yl(1)], ...
    ...
    results = [results sprintf('%s > %s \\t%d to %d\\t %0.3f\\n', ...
                  ...
                  xaxis(pos_true.froms(c)), xaxis(pos_true.tos(c)), ...
                  pos_true.cluster_p(c))];
...
if isempty(pos_true.froms) && isempty(neg_true.froms)
