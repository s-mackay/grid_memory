% figure2 - this function will plot one of the 4 panels as seen in figure 2
% 
%   INPUTS:
%                id (int): row number within spiketable of unit to plot
%        img_or_pos (int): plot image response, 2: plot location response
%       spikecell (table): optoinal. Contains the experimental data and spike times

function figure2(data_dir, id, item_or_loc, spiketable)
...
% corr_trials  = spiketable.successful_tap{id} &  spiketable.subsequent_memory{id};
% false_trials = spiketable.successful_tap{id} & ~spiketable.subsequent_memory{id};
binsize     = 100;
...
for m = 1:nstims % e.g. 1:8 different stimuli or 1:9 positions
    tr_img = find(spiketable.(st_c){id} == m & ...
        spiketable.successful_tap{id} == 1); % trial indices for image
    hold on
    rmbrd_all{2,m} = [];
    false_all{2,m} = [];
...
% catch
%     fprintf('%s unit %i: subs rem vs forg, p=%.3g\n',...
%         stim_type, id, p);
%     fprintf('    too few spikes fired to compute z value\n');
% end
...
%print(gcf, '-depsc', '-r600', [fname(1:end-3), 'eps']);
%print('-painters', gcf, '-depsc', '-r600', [fname(1:end-3), 'eps']);
